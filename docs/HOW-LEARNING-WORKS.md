...
The agent calls tools to capture what it observed. The base iteration limit is 5 per turn, but dynamically extends up to 10 when the agent is actively exploring code (each `search_code`, `read_file`, or `list_directory` call adds one iteration). The agent can call multiple tools and then respond, or alternate between tools and text.
...
| `query_teaching_moments` | Searches for relevant industry patterns |
| `query_case_studies` | Searches for real-world incidents to reference in conversation |

**Cross-practice tools:**

| Tool | What it does |
|------|-------------|
| `suggest_experiment` | Recommends chaos experiments, load tests, or gamedays based on gaps |
| `record_dependency` | Maps service dependencies as they surface in conversation |
| `record_action_item` | Creates structured follow-ups with owner, priority, and due date |
| `suggest_cross_practice_action` | Links findings to other practices (incident analysis, chaos engineering) |

**Code exploration tools (ORR only, when a repository is configured):**

| Tool | What it does |
|------|-------------|
| `search_code` | Searches the service's source code for patterns |
| `read_file` | Reads a specific file from the repository |
